    ...
    public abstract class SteamPeer
    {
            ...
            if (messageCount > 0)
            {
                for (int i = 0; i < messageCount; i++)
                {
                    ...
                    if (data.m_cbSize > 0)
                    {
                        ...
                        {
                            GD.PrintErr($"{LogName}: Can't fully handle {data.m_cbSize} bytes because it exceeds the maximum of {receiveBuffer.Length}. Data will be incomplete!");
        ...
        internal void Send(byte[] dataBuffer, int numBytes, HSteamNetConnection toConnection)
        {
            ...
            if (result != EResult.k_EResultOK)
                GD.PrintErr($"{LogName}: Failed to send {numBytes} bytes - {result}");
